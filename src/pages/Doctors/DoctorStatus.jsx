...
const COLUMNS = [
  ...
  { key: 'statusDetail', label: 'Detail', type: 'text' },
];

const DoctorStatus = () => {
  const { data, loading, error } = useApiData(async () => {
    const payload = await apiRequest('/api/doctors/verification');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      {loading ? (
        <div style={{ padding: '12px 0', color: '#475569' }}>Loading doctor verifications...</div>
      ) : (
        <AdminTable title="Doctor Status" columns={COLUMNS} initialData={tableData} />
      )}
    </div>
  );
};
